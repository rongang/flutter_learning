...
/// call. Call [SearchDelegate.close] before re-using the same delegate instance
/// for another [showSearch] call.
abstract class MySearchDelegate<T> extends SearchDelegate {
...
class _SearchAPage<T> extends StatefulWidget {
  const _SearchAPage({
    this.delegate,
    this.animation,
  });
  ...
  @override
  State<StatefulWidget> createState() => _SearchAPageState<T>();
}

class _SearchAPageState<T> extends State<_SearchAPage<T>> {
  ...
  @override
  void didUpdateWidget(_SearchAPage<T> oldWidget) {
