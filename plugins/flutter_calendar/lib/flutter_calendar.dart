...
class _CalendarState extends State<Calendar> {
  ...
  Widget get calendarGridView {
    return new Container(
      child: Listener(
        ...
        onPointerUp: (point) {
          if((point.position.dx - gestureStart.position.dx).abs()<50) return ;
          endSwipe();
        },
        onPointerMove: (point) {
          getDirection(point.position.dx);
        },
  ...
  void beginSwipe(PointerDownEvent point) {
    gestureStart = point;
  }

  void getDirection(double dx) {
    if (dx < gestureStart.position.dx) {
