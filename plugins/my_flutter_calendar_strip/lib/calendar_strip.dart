...
class CalendarStripState extends State<CalendarStrip>
  ...
  List<String> dayLabels_zh = ["一", "二", "三", "四", "五", "六", "七"];
  ...
  void initState() {
    ...
    dateRange = calculateDateRange(null);
  ...
  String getMonthName(
    ...
    return widget.locale.languageCode == 'zh'
        ? monthLabels_zh[dateObj.month - 1]
        : monthLabels[dateObj.month - 1];
  }
  ...
  String getMonthLabel() {
    ...
    if (startingDayObj.month == endingDayObj.month) {
      label = "${startingDayObj.year} ${getMonthName(startingDayObj)}";
    } else {
      var startingDayYear =
          "${startingDayObj.year == endingDayObj.year ? "" : startingDayObj.year}";
      label =
          "$startingDayYear ${getMonthName(startingDayObj)} / ${endingDayObj.year} ${getMonthName(endingDayObj)}";
    }
    return label;
  }
  ...
  Widget dateTileBuilder(DateTime date, DateTime selectedDate, int rowIndex) {
    ...
    String dayName = widget.locale.languageCode == 'zh'
        ? dayLabels_zh[date.weekday - 1]
